...
## 1. 项目概述
### 1.1 项目背景
本项目旨在参考主流社交媒体平台（如微博、小红书），设计并实现一个模拟的校内社交互动平台。核心目标是为大学生提供一个集内容发布、互动交流、个人展示为一体的Web应用。
...
* **设计风格**：年轻化、扁平化、简洁大气。
* **主色调**：建议采用 *校园蓝* (Primary) 或 *活力橙* 作为强调色，配合 Element Plus 默认的白色/灰色背景。
* **布局模式**：
    * PC端：经典三栏布局（左侧导航/用户信息，中间信息流，右侧推荐/热门）。
...
* **数据栏**：关注数、粉丝数、获赞数。
* **操作按钮**：
    * 如果是他人主页：[关注/取消关注]、[私信(可选 UI)]。
    * 如果是自己主页：[编辑资料]。
* **内容Tab**：[TA的动态]、[相册(可选)]、[收藏(可选)]。

**关注机制：**
* 首页 Feed 流支持切换视图：[全站动态] vs [关注动态]。
* 交互：点击关注后，按钮状态即时变为“已关注” (前端乐观更新)。

### 3.5 管理员后台 (Admin Dashboard) - *独立/嵌入路由*
* **用户管理表格 (`ElTable`)**：
